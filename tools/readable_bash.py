...
<html>
    ...
    <div class="container">
      <div class="row intro">
        ...
        <p>This document is generated automatically from <a href="https://github.com/wtarit/s5mail">S5 Mail</a>&rsquo;s setup script and source code.</p>
        <hr>
      </div>
    </div>
...
class OtherLine(Grammar):
	...
	def value(self):
		...
		if "source /etc/s5mail.conf" in self.string: return ""
		return "<pre class='shell'><div>" + recode_bash(self.string.strip()) + "</div></pre>\n"
...
class BashScript(Grammar):
	...
	@staticmethod
	def parse(fn):
		if fn in {"setup/functions.sh", "/etc/s5mail.conf"}: return ""
		...
		return v.replace("`pwd`",  "<code><b>/path/to/s5mail</b></code>")

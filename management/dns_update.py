...
import sys, os, os.path, datetime, re, hashlib, base64, grp
import ipaddress
import rtyaml
import dns.resolver
...
from smtp_relay import should_generate_direct_delivery_spf
...
def do_dns_update(env, force=False):
	...
	# Keep Rspamd's allowed signing domains aligned with mail users and aliases.
	# This is the same domain lifecycle that drives the DNS zones above.
	from mailconfig import get_mail_domains
	if write_rspamd_dkim_tables(get_mail_domains(env), env):
		if os.path.exists("/usr/bin/rspamadm"):
			shell('check_call', ["/usr/bin/rspamadm", "configtest"])
			shell('check_call', ["/usr/sbin/service", "rspamd", "reload"], trap=True)
		if len(updated_domains) == 0:
			updated_domains.append("Rspamd DKIM signing configuration")
	...
	if len(updated_domains) == 0:
		# If nothing was updated, don't show any output.
		return ""
...
def build_zone(domain, domain_properties, additional_records, env, is_zone=True):
	...
	if domain_properties[domain]["mail"]:
		...
		# the domain, and no one else. When an SMTP relay is enabled, its sending
		# infrastructure requires a provider-specific SPF record, so do not
		# generate the direct-delivery default. Custom SPF records are preserved.
		if should_generate_direct_delivery_spf(env) and not has_rec(None, "TXT", prefix="v=spf1 "):
			records.append((None,  "TXT", 'v=spf1 mx -all', f"Recommended. Specifies that only the box is permitted to send @{domain} mail."))

		# Append the DKIM TXT record generated during S5 provisioning.
	...
	if domain_properties[domain]["mail"] \
	  ...
	  and is_domain_cert_signed_and_valid("mta-sts." + domain, env):
		...
		with open("/var/lib/s5mail/mta-sts.txt", "rb") as f:
...
def build_tlsa_record(env):
	...
	# generate a private key once the first time S5 Mail is set up
...
def write_rspamd_dkim_tables(domains, env):
	"""Write Rspamd's managed-domain signing tables without exposing the key."""
	config_dir = "/etc/rspamd/s5mail-dkim"
	key_file = os.path.join(env['STORAGE_ROOT'], 'mail/dkim/mail.private')
	if not os.path.isdir(config_dir) or not os.path.exists(key_file):
		return False

	domains = sorted(set(domains))
	config = {
		"SigningTable": "".join(f"*@{domain} {domain}\n" for domain in domains),
		"KeyTable": "".join(f"{domain} {domain}:mail:{key_file}\n" for domain in domains),
	}
	did_update = False
	for filename, content in config.items():
		...
		os.chown(temporary_path, 0, grp.getgrnam("_rspamd").gr_gid)
